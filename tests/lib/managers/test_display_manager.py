...
class TestDisplay:
    class TestNormalScreen:
        def test_device_changed_cycles_active_remote(self, event_bus, remote_names):
            ...
            assert_text_called_with_specific_text_once(
                display._screen.frame_buffer, "Remote 1"
            )
        ...
        def test_device_added_event_displays_device_in_normal_screen_menu(
            self, event_bus, remote_names
        ):
            display = Display(event_bus, remote_names=remote_names, protocol_name="NEC")
    ...
    class TestAllScreens:

        def test_battery_level_changed_event_updates_battery_icon(
            self, event_bus, remote_names
        ):
            display = Display(
                event_bus,
            ...
            event_bus.publish(Events.BATTERY_LEVEL_CHANGED, 2)

            assert display._battery_charge == 2

        def test_low_battery_event_displays_empty_battery_icon(
            self, event_bus, remote_names
        ):
            display = Display(
                event_bus,
            ...
            event_bus.publish(Events.LOW_BATTERY)

            assert display._battery_charge == 0
